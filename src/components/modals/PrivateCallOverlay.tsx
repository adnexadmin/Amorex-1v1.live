...
export const PrivateCallOverlay: React.FC<PrivateCallOverlayProps> = ({
  ...
  const [currentRateText, setCurrentRateText] = useState<string>('1x (300 🪙)');

  const connectedAtRef = useRef<number | null>(null);
  const chargedSlabsRef = useRef<Set<number>>(new Set());
  const totalCoinsDeductedRef = useRef<number>(0);

  // Guardrail: Minimum 300 coins required to initiate call
  useEffect(() => {
    if ((user?.coins ?? 0) < 300) {
      sound.playAlert();
      alert("Insufficient Balance! Minimum 300 coins required to initiate 1v1 Video Call.");
      onEndCall(0, 0);
    }
  }, [user?.coins, onEndCall]);
  ...
  // Dynamic 45-Second Multiplier Calculator
  const getSlabCost = (slabIndex: number): { amount: number; label: string } => {
    if (slabIndex === 1) return { amount: 300, label: '1x (300 🪙)' };
  ...
  // Heavy-duty Coin Deduction Engine
  useEffect(() => {
    ...
    if (connectedAtRef.current === null) {
      ...
      // Initial deduction (300 Coins) for first 45-second slab upon connection
    ...
    const interval = setInterval(() => {
      if (connectedAtRef.current) {
        ...
        // Calculate current 45-second slab index
        ...
        if (!chargedSlabsRef.current.has(activeSlab)) {
          ...
          if (!success) {
            sound.playAlert();
            alert("Insufficient Coins! Call disconnected automatically.");
            closeRtc();
  ...
  }, [callState, onDeductCoins, closeRtc, onEndCall]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-between select-none overflow-hidden font-sans w-screen h-screen">
      ...
      {/* Top Bar with Live Rate Multiplier Badge */}
